...
export function PersonalizeChatWindow(props: {
  ...
  const { emptyStateComponent, placeholder, showIngestForm, showIntermediateStepsToggle, emoji } = props;
  ...
  async function sendMessage(e: FormEvent<HTMLFormElement>) {
    ...
    // Some extra work to show intermediate steps properly
      ...
      const response = await fetch('/api/chat/personalize', {
        method: "POST",
        body: JSON.stringify({
  ...
  return (
    <div className={`flex flex-col items-center p-4 md:p-8 rounded grow overflow-hidden ${(messages.length > 0 ? "border" : "")}`}>
      <h2 className={`${messages.length > 0 ? "" : "hidden"} text-2xl`}>{emoji} Hello</h2>
